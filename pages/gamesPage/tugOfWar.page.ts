...
export default class tugOfWarPage {
        ...
        async enableMainboardMessageCheckbox() {
                let ele = await this.page.frameLocator('iframe').locator("(//input[@type='checkbox'])[3]").isVisible()
                if ((ele == true)) {
        ...
        async inputUrlRedirectMassage() {
                let ele = await this.page.frameLocator('iframe').locator("//textarea[@placeholder='Type message']")
        ...
        async clickStagesBtn() {
                await this.page.frameLocator('iframe').locator("text=AutoStartStagesGame DesignDelete >> p").first().click()
        ...
        async clickGameDeleteBtn() {
                await this.page.frameLocator('iframe').locator('text=AutoStartStagesGame DesignDelete >> p').nth(2).click();
